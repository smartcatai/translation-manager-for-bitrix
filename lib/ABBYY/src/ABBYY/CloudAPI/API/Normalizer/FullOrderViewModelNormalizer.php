...
class FullOrderViewModelNormalizer extends SerializerAwareNormalizer implements DenormalizerInterface, NormalizerInterface
{
    public function supportsDenormalization($data, $type, $format = null)
    {
        if ($type !== 'ABBYY\\CloudAPI\\API\\Model\\FullOrderViewModel') {
            return false;
        }
        return true;
    }
    ...
    {
        if ($data instanceof \Smartcat\ConnectorAPI\API\Model\FullOrderViewModel) {
            return true;
        }
        return false;
    }
    public function denormalize($data, $class, $format = null, array $context = array())
    {
        ...
        $object = new \Smartcat\ConnectorAPI\API\Model\FullOrderViewModel();
        ...
        if (property_exists($data, 'translations')) {
            ...
            foreach ($data->{'translations'} as $value) {
                $values[] = $this->serializer->deserialize($value, 'ABBYY\\CloudAPI\\API\\Model\\TranslationViewModel', 'raw', $context);
            }
        ...
        if (property_exists($data, 'units_count')) {
            ...
            foreach ($data->{'units_count'} as $key => $value_2) {
                $values_2[$key] = $value_2;
            }
            $object->setUnitsCount($values_2);
        }
    ...
    {
        $data = new \stdClass();
        if (null !== $object->getTranslations()) {
            $values = array();
            foreach ($object->getTranslations() as $value) {
        ...
        if (null !== $object->getUnitsCount()) {
            ...
            foreach ($object->getUnitsCount() as $key => $value_2) {
                $values_2->{$key} = $value_2;
            }
            $data->{'units_count'} = $values_2;
        }
